...
class cocktail_core_css_parsers_CSSRulesParser {
		...
		while(!($c === 0)) {
			...
			case 6:
			{
				if($c !== 42) {
					...
					$state = cocktail_core_css_parsers_StyleSheetRulesParserState::$COMMENT;
				}
			}break;
			case 5:
			{
				if($c === 42) {
					$state = cocktail_core_css_parsers_StyleSheetRulesParserState::$END_COMMENT;
				}
			}break;
			case 7:
			{
	...
	static function isValidChar($c) {
		...
		$製pos = $GLOBALS['%s']->length;
		{
			$裨mp = $c >= 97 && $c <= 122 || $c >= 65 && $c <= 90 || $c >= 48 && $c <= 57 || $c === 58 || $c === 46 || $c === 95 || $c === 45;
			$GLOBALS['%s']->pop();
			return $裨mp;
		}
		$GLOBALS['%s']->pop();

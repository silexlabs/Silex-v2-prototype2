	...
	static function parseStyleValue($propertyName, $styles, $position) {
		...
		if($styleValuesLists->length === 0) {
			if($styleValues->length === 1) {
				$»tmp = new cocktail_core_css_TypedPropertyVO($propertyName, $styleValues[0], $important);
				$GLOBALS['%s']->pop();
				return $»tmp;
			} else {
				$»tmp = new cocktail_core_css_TypedPropertyVO($propertyName, cocktail_core_css_CSSPropertyValue::GROUP($styleValues), $important);
				$GLOBALS['%s']->pop();
				return $»tmp;
			}
		} else {
			...
			{
				$»tmp = new cocktail_core_css_TypedPropertyVO($propertyName, cocktail_core_css_CSSPropertyValue::CSS_LIST($styleListProperty), $important);
				$GLOBALS['%s']->pop();
				return $»tmp;
			}
		}

...
class cocktail_core_css_parsers_CSSStyleParser {
	public function __construct() { if(!php_Boot::$skip_constructor) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::new");
		$»spos = $GLOBALS['%s']->length;
		$GLOBALS['%s']->pop();
	}}
	static $_position = 0;
	static $_typedProperties;
	static function parseStyle($styles) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseStyle");
		$»spos = $GLOBALS['%s']->length;
		...
		if(cocktail_core_css_parsers_CSSStyleParser::$_typedProperties === null) {
			cocktail_core_css_parsers_CSSStyleParser::$_typedProperties = new _hx_array(array());
		}
		cocktail_core_css_parsers_CSSStyleParser::$_typedProperties = cocktail_core_utils_Utils::clear(cocktail_core_css_parsers_CSSStyleParser::$_typedProperties);
		$typedProperties = cocktail_core_css_parsers_CSSStyleParser::$_typedProperties;
		$position = 0;
		$start = $position;
	...
	static function parseStyleValue($propertyName, $styles, $position) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseStyleValue");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		if($c === 58) {
			++$position;
		...
		if($styleValuesLists->length === 0) {
			if($styleValues->length === 1) {
				...
				$typedProperty->important = $important;
				$typedProperty->name = $propertyName;
				$typedProperty->typedValue = cocktail_core_css_CSSPropertyValue::GROUP($styleValues);
				{
					$GLOBALS['%s']->pop();
					return $typedProperty;
				}
			...
			$typedProperty->important = $important;
			$typedProperty->name = $propertyName;
			$typedProperty->typedValue = cocktail_core_css_CSSPropertyValue::CSS_LIST($styleListProperty);
			{
				$GLOBALS['%s']->pop();
				return $typedProperty;
			}
	...
	static function parseImportant($styles, $position) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseImportant");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		$start = $position;
	...
	static function parseIntegerNumberDimensionOrPercentage($styles, $position, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseIntegerNumberDimensionOrPercentage");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		$start = $position;
		if($c === 45) {
	...
	static function parseDimension($numberOrInteger, $styles, $position, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseDimension");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		$start = $position;
		while($c >= 97 && $c <= 122 || $c >= 65 && $c <= 90 || $c === 45) {
	...
	static function parseString($styles, $position, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseString");
		$»spos = $GLOBALS['%s']->length;
	...
	static function parseIdentOrFunctionnalNotation($styles, $position, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseIdentOrFunctionnalNotation");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		$start = $position;
		while($c >= 97 && $c <= 122 || $c >= 65 && $c <= 90 || $c === 45) {
	...
	static function parseHexaColor($styles, $position, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseHexaColor");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		$start = $position;
	...
	static function parseFunctionnalNotation($ident, $styles, $position, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseFunctionnalNotation");
		$»spos = $GLOBALS['%s']->length;
		$c = ord(substr($styles,$position,1));
		$start = $position;
		while($c !== 41) {
	...
	static function getFunctionalNotation($name, $value) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::getFunctionalNotation");
		$»spos = $GLOBALS['%s']->length;
		switch($name) {
		case "rgb":{
			$»tmp = cocktail_core_css_parsers_CSSStyleParser::parseRGBOrRGBA($value, false);
			$GLOBALS['%s']->pop();
			return $»tmp;
		}break;
		case "rgba":{
			$»tmp = cocktail_core_css_parsers_CSSStyleParser::parseRGBOrRGBA($value, true);
			$GLOBALS['%s']->pop();
			return $»tmp;
		}break;
		case "url":{
			...
			$value1 = $»t->params[0];
			{
				$»tmp = cocktail_core_css_CSSPropertyValue::URL($value1);
				$GLOBALS['%s']->pop();
				return $»tmp;
			}break;
			case 6:
	...
	static function parseRGBOrRGBA($property, $isRGBA) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseRGBOrRGBA");
		$»spos = $GLOBALS['%s']->length;
		$»t = ($property);
		switch($»t->index) {
		case 14:
		$value = $»t->params[0];
		{
			...
			if($isRGBA === false) {
				if($isPercentRGB === true) {
					$»tmp = cocktail_core_css_CSSPropertyValue::COLOR(cocktail_core_css_CSSColorValue::RGB_PERCENTAGE($percentRed, $percentGreen, $percentBlue));
					$GLOBALS['%s']->pop();
					return $»tmp;
				} else {
					$»tmp = cocktail_core_css_CSSPropertyValue::COLOR(cocktail_core_css_CSSColorValue::RGB($red, $green, $blue));
					$GLOBALS['%s']->pop();
					return $»tmp;
				}
			} else {
				...
				if($isPercentRGB === true) {
					$»tmp = cocktail_core_css_CSSPropertyValue::COLOR(cocktail_core_css_CSSColorValue::RGBA_PERCENTAGE($percentRed, $percentGreen, $percentBlue, $alpha));
					$GLOBALS['%s']->pop();
					return $»tmp;
				} else {
					$»tmp = cocktail_core_css_CSSPropertyValue::COLOR(cocktail_core_css_CSSColorValue::RGBA($red, $green, $blue, $alpha));
					$GLOBALS['%s']->pop();
					return $»tmp;
				}
			}
	...
	static function parseIdent($ident, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseIdent");
		$»spos = $GLOBALS['%s']->length;
	...
	static function parseKeyword($ident, $styleValues) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseKeyword");
		$»spos = $GLOBALS['%s']->length;
	...
	static function parseColorKeyword($ident) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseColorKeyword");
		$»spos = $GLOBALS['%s']->length;
	...
	static function parseColorKeyword2($ident) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::parseColorKeyword2");
		$»spos = $GLOBALS['%s']->length;
		$cssPropertyValue = null;
		switch($ident) {
		case "lightyellow":{
	...
	static function isHexaChar($c) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::isHexaChar");
		$»spos = $GLOBALS['%s']->length;
		{
			$»tmp = $c >= 97 && $c <= 102 || $c >= 65 && $c <= 70 || $c >= 48 && $c <= 57;
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();
	...
	static function isIdentChar($c) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::isIdentChar");
		$»spos = $GLOBALS['%s']->length;
		{
			$»tmp = $c >= 97 && $c <= 122 || $c >= 65 && $c <= 90 || $c === 45;
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();
	...
	static function isNumChar($c) {
		$GLOBALS['%s']->push("cocktail.core.css.parsers.CSSStyleParser::isNumChar");
		$»spos = $GLOBALS['%s']->length;
		{
			$»tmp = $c >= 48 && $c <= 57;
			$GLOBALS['%s']->pop();

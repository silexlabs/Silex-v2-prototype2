...
class cocktail_core_layout_computer_VisualEffectStylesComputer {
	...
	static function getComputedTransformOrigin($style) {
		...
		$퍁 = (_hx_deref((cocktail_core_layout_computer_VisualEffectStylesComputer_0($style, $transformOriginX, $transformOriginY, $x, $y)))->typedValue);
		switch($퍁->index) {
		case 13:
		$value = $퍁->params[0];
		{
	...
	static function getComputedTransform($style) {
		...
		$퍀pos = $GLOBALS['%s']->length;
		if($style->isNone(_hx_deref((cocktail_core_layout_computer_VisualEffectStylesComputer_1($style)))->typedValue)) {
			$퍁mp = new cocktail_core_geom_Matrix(null, null, null, null, null, null);
			$GLOBALS['%s']->pop();
			return $퍁mp;
		}
		...
		$퍁 = (_hx_deref((cocktail_core_layout_computer_VisualEffectStylesComputer_2($matrix, $style, $transformFunctions, $transformOrigin)))->typedValue);
		switch($퍁->index) {
		case 13:
		$value = $퍁->params[0];
		{
		...
		$matrix->translate($transformOrigin->x, $transformOrigin->y);
		{
			...
			while($_g1 < $_g) {
				...
				switch($퍁->index) {
				case 0:
				$f = $퍁->params[5]; $e = $퍁->params[4]; $d = $퍁->params[3]; $c = $퍁->params[2]; $b = $퍁->params[1]; $a = $퍁->params[0];
				{
...
function cocktail_core_layout_computer_VisualEffectStylesComputer_0(&$style, &$transformOriginX, &$transformOriginY, &$x, &$y) {
	$퍀pos = $GLOBALS['%s']->length;
	{
		...
		$length = $_this->_properties->length;
		{
			...
			while($_g < $length) {
				$i = $_g++;
				if(_hx_array_get($_this->_properties, $i)->name === "transform-origin") {
...
function cocktail_core_layout_computer_VisualEffectStylesComputer_1(&$style) {
	$퍀pos = $GLOBALS['%s']->length;
	{
		$_this = $style->computedValues;
		$typedProperty = null;
...
function cocktail_core_layout_computer_VisualEffectStylesComputer_2(&$matrix, &$style, &$transformFunctions, &$transformOrigin) {
	$퍀pos = $GLOBALS['%s']->length;
	{
		$_this = $style->computedValues;
		$typedProperty = null;

...
class cocktail_core_layout_floats_FloatsManager {
	...
	public function registerFloat($elementRenderer, $currentFormattingContextY, $currentFormattingContextX, $containingBlockWidth) {
		...
		$»t = ($coreStyle->getKeyword(_hx_deref((cocktail_core_layout_floats_FloatsManager_0($this, $containingBlockWidth, $coreStyle, $currentFormattingContextX, $currentFormattingContextY, $elementRenderer, $ret)))->typedValue));
		switch($»t->index) {
		case 11:
		{
...
function cocktail_core_layout_floats_FloatsManager_0(&$»this, &$containingBlockWidth, &$coreStyle, &$currentFormattingContextX, &$currentFormattingContextY, &$elementRenderer, &$ret) {
	$»spos = $GLOBALS['%s']->length;
	{

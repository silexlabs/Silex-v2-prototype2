...
class org_silex_core_Silex {
	...
	static function main() {
		$serverConfig = new org_silex_core_ServerConfig(null);
		$publicationService = new org_silex_publication_PublicationService();
		if(haxe_remoting_HttpConnection::handleRequest(org_silex_service_ServiceBase::$context)) {
			return;
		}
		...
		if(org_silex_core_Silex::$publicationName === "") {
			org_silex_core_Silex::$publicationName = $serverConfig->defaultPublication;
		}
		$publicationData = $publicationService->getPublicationData(org_silex_core_Silex::$publicationName, null);
		$publicationConfig = $publicationService->getPublicationConfig(org_silex_core_Silex::$publicationName, null);
		$initialPageName = "";
		if($params->length === 1) {
			$initialPageName = "";
		} else {
		...
		org_slplayer_util_DomTools::setMeta("publicationName", org_silex_core_Silex::$publicationName, null);

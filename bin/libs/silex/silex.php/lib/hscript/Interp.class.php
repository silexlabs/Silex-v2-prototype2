...
class hscript_Interp {
	...
	public function expr($e) {
		...
		$name = $»t->params[2]; $fexpr = $»t->params[1]; $params = $»t->params[0];
		{
			...
			$me = $this;
			$f = array(new _hx_lambda(array(&$capturedLocals, &$e, &$fexpr, &$me, &$name, &$params), "hscript_Interp_1"), 'execute');
...
function hscript_Interp_1(&$capturedLocals, &$e, &$fexpr, &$me, &$name, &$params, $args) {
	$»spos = $GLOBALS['%s']->length;
	{
...
function hscript_Interp_2(&$»this, &$delta, &$e, &$id, &$l, &$prefix) {
	$»spos = $GLOBALS['%s']->length;
	if($l === null) {
...
function hscript_Interp_8(&$me, $e1, $e2) {
	$»spos = $GLOBALS['%s']->length;
	{
		...
		$»spos2 = $GLOBALS['%s']->length;
		{
			$»tmp = _hx_mod($me->expr($e1), $me->expr($e2));
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();

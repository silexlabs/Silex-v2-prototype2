...
class cocktail_core_html_HTMLBodyElement extends cocktail_core_html_HTMLElement {
	...
	public function cascadeSelf($parentChangedProperties, $programmaticChange) {
		...
		$»spos = $GLOBALS['%s']->length;
		$changedProperties = parent::cascadeSelf($parentChangedProperties,$programmaticChange);
		$parentCoreStyle = $this->parentNode->coreStyle;
		if($parentCoreStyle->computedValues->get_overflowX() !== null) {
			$»t = ($parentCoreStyle->getKeyword($parentCoreStyle->get_overflowX()));
			switch($»t->index) {
			case 36:
			{
		...
		if($parentCoreStyle->computedValues->get_overflowY() !== null) {
			$»t = ($parentCoreStyle->getKeyword($parentCoreStyle->get_overflowY()));
			switch($»t->index) {
			case 36:
			{

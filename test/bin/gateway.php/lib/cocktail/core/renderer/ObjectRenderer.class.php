...
class cocktail_core_renderer_ObjectRenderer extends cocktail_core_renderer_EmbeddedBoxRenderer {
	...
	public function renderEmbeddedAsset($graphicContext) {
		...
		$viewport = $this->get_globalBounds();
		$viewport->x += $this->coreStyle->usedValues->paddingLeft;
		$viewport->y += $this->coreStyle->usedValues->paddingTop;
		$htmlObjectElement->plugin->set_viewport($viewport);
		$GLOBALS['%s']->pop();
	}
	public function createLayer($parentLayer) {
		$GLOBALS['%s']->push("cocktail.core.renderer.ObjectRenderer::createLayer");
		$製pos = $GLOBALS['%s']->length;

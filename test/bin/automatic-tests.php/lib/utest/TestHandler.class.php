...
class utest_TestHandler {
	...
	public function addEvent($f, $timeout = null) {
		...
		return array(new _hx_lambda(array(&$f, &$handler, &$timeout), "utest_TestHandler_0"), 'execute');
	}
	public function addAsync($f, $timeout = null) {
		if($timeout === null) {
			$timeout = 250;
		}
		if(null === $f) {
			$f = array(new _hx_lambda(array(&$f, &$timeout), "utest_TestHandler_1"), 'execute');
		}
		$this->asyncStack->add($f);
		$handler = $this;
		$this->setTimeout($timeout);
		return array(new _hx_lambda(array(&$f, &$handler, &$timeout), "utest_TestHandler_2"), 'execute');
	}
	public function unbindHandler() {
		utest_Assert::$results = null;
		utest_Assert::$createAsync = array(new _hx_lambda(array(), "utest_TestHandler_3"), 'execute');
		utest_Assert::$createEvent = array(new _hx_lambda(array(), "utest_TestHandler_4"), 'execute');
	}
	...
	public function setTimeout($timeout) {
		...
		$this->expireson = utest_TestHandler_5($this, $newexpire, $timeout);
	}
	public $expireson;
...
function utest_TestHandler_0(&$f, &$handler, &$timeout, $e) {
	{
...
function utest_TestHandler_1(&$f, &$timeout) {
	{
	}
}
function utest_TestHandler_2(&$f, &$handler, &$timeout) {
	{
		if(!$handler->asyncStack->remove($f)) {

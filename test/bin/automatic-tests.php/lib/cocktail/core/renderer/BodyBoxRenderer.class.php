...
class cocktail_core_renderer_BodyBoxRenderer extends cocktail_core_renderer_BlockBoxRenderer {
	...
	public function getBackgroundBounds() {
		...
		return new cocktail_core_geom_RectangleVO(0.0, 0.0, $width, $height);
	}
	public function layoutSelf() {
		parent::layoutSelf();
		if($this->coreStyle->isAuto($this->coreStyle->get_height()) === true && ($this->isPositioned() === false || $this->isRelativePositioned() === true)) {
			$usedValues = $this->coreStyle->usedValues;
			$usedValues->height = $this->_containingBlock->getContainerBlockData()->height - $usedValues->marginTop - $usedValues->marginBottom - $usedValues->paddingTop - $usedValues->paddingBottom;
		}
	}

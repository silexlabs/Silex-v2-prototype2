...
class org_silex_config_PublicationConfigManager extends org_silex_config_ConfigBase {
	public function __construct($configFile = null) {
		...
		$this->publicationConfig = _hx_anonymous(array("state" => org_silex_publication_PublicationState::$Private, "creation" => _hx_anonymous(array("author" => "", "date" => new Date(0, 0, 0, 0, 0, 0))), "lastChange" => _hx_anonymous(array("author" => "", "date" => new Date(0, 0, 0, 0, 0, 0)))));
	...
	public function getChangeDataFromXML($xml) {
		...
		if($xml->hasNode->resolve("author")) {
			...
			if($xml->has->resolve("author")) {
				...
				haxe_Log::trace("Warning: missing author in config file ", _hx_anonymous(array("fileName" => "PublicationConfigManager.hx", "lineNumber" => 107, "className" => "org.silex.config.PublicationConfigManager", "methodName" => "getChangeDataFromXML")));
			}
		}
		$date = null;
		if($xml->hasNode->resolve("date")) {
			...
			if($xml->has->resolve("date")) {
				...
				haxe_Log::trace("Warning: missing date in config file ", _hx_anonymous(array("fileName" => "PublicationConfigManager.hx", "lineNumber" => 117, "className" => "org.silex.config.PublicationConfigManager", "methodName" => "getChangeDataFromXML")));
			}
		}
	...
	public function xmlToConfData($xml) {
		if($xml->hasNode->resolve("creation")) {
			...
			haxe_Log::trace("Warning: missing creation in config file ", _hx_anonymous(array("fileName" => "PublicationConfigManager.hx", "lineNumber" => 64, "className" => "org.silex.config.PublicationConfigManager", "methodName" => "xmlToConfData")));
		}
		if($xml->hasNode->resolve("lastChange")) {
			$this->publicationConfig->lastChange = $this->getChangeDataFromXML($xml->node->resolve("lastChange"));
		} else {
			haxe_Log::trace("Warning: missing lastChange in config file ", _hx_anonymous(array("fileName" => "PublicationConfigManager.hx", "lineNumber" => 69, "className" => "org.silex.config.PublicationConfigManager", "methodName" => "xmlToConfData")));
		}
		if($xml->hasNode->resolve("state")) {
			...
			haxe_Log::trace("Warning: missing state in config file ", _hx_anonymous(array("fileName" => "PublicationConfigManager.hx", "lineNumber" => 90, "className" => "org.silex.config.PublicationConfigManager", "methodName" => "xmlToConfData")));
		}
	}

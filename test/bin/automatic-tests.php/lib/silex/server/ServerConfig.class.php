...
class silex_server_ServerConfig extends silex_ConfigBase {
	...
	public function xmlToConfData($xml) {
		if($xml->hasNode->resolve("defaultPublication")) {
			...
			haxe_Log::trace("Warning: missing defaultPublication in config file ", _hx_anonymous(array("fileName" => "ServerConfig.hx", "lineNumber" => 46, "className" => "silex.server.ServerConfig", "methodName" => "xmlToConfData")));
		}
		if($xml->hasNode->resolve("admin")) {
			$this->admin = $xml->node->resolve("admin")->getInnerData();
		} else {
			haxe_Log::trace("Warning: missing admin in config file ", _hx_anonymous(array("fileName" => "ServerConfig.hx", "lineNumber" => 51, "className" => "silex.server.ServerConfig", "methodName" => "xmlToConfData")));
		}

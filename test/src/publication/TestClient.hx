...
<lastChange>
	...
	public function new(){
		...
		ServiceBase.GATEWAY_URL = AllTestsClient.GATEWAY_URL;
	...
	public function testRead():Void{
		publicationService.getPublicationData("test-publication-private", 
			Assert.createEvent(onResultRead, 2000)
			, onError
		);
		publicationService.getPublicationData("test-not-exist", 
			onResultRead
	...
	public function testGetPublications():Void{
		publicationService.getPublications([Private], null, cast(Assert.createEvent(onResultGetPublications)));
	}
	...
	public function testGetThemes():Void{
		publicationService.getPublications(null, [Theme], cast(Assert.createEvent(onResultGetThemes)));
	}
	...
	public function testWrite():Void{
		...
		publicationService.setPublicationData("test-write", TEST_PUBLICATION_DATA
			, createAsyncCallback(THIS_TEST_PATH + "test-write/app.css",TEST_CSS)
			, onError
		);
	}
	...
	public function testCreateAndTrash():Void{
		...
		publicationService.create("test-create-tmp", TEST_PUBLICATION_DATA
			, createAsyncCallback(THIS_TEST_PATH + "test-create-tmp/app.css",TEST_CSS)
			, onError
		);
	...
	private function doEmptyTrash():Void{
		...
		publicationService.emptyTrash(
			Assert.createAsync(onResultWriteNoCheck, 4000)
			//createAsyncCallback(THIS_TEST_PATH + "test-create/app.css", "Not Found")
			, onError
		);
	}
	public function testDuplicateAndRename():Void{
		...
		publicationService.duplicate("test-duplicate", "duplicated-tmp"
			, createAsyncCallback(THIS_TEST_PATH + "duplicated-tmp/app.css",TEST_CSS)
			, onError
		);
	...
	private function doRename():Void{
		trace("doRename");
		publicationService.rename("duplicated-tmp", "renamed-tmp"
			, createAsyncCallback(THIS_TEST_PATH + "renamed-tmp/app.css",TEST_CSS)
			, onError
		);

...
class Layer extends DisplayObject{
	...
	public function detectTransition(transitionData:TransitionData) : Bool {
		...
		var event:CustomEvent = cast(Lib.document.createEvent("Event"));
		event.initEvent(TransitionData.EVENT_TYPE_REQUEST, true, true);
		untyped event.data = transitionData;
	...
	public function doHide(nullIfCalledDirectly:Dynamic=null) : Void {
		...
		while (rootElement.childNodes.length > 0){
			...
			if(element.tagName!= null 
				&& (element.tagName.toLowerCase() == "audio" || element.tagName.toLowerCase() == "video")
				){
				try{				
					cast(element).pause();
